...
def control_change(control_number, value)
  $file.write([(CONTROL_CHANGE | PRA32_U2_MIDI_CH), control_number, value].pack("C*"))
  wait(10)
end

def program_change(program_number)
  $file.write([(PROGRAM_CHANGE | PRA32_U2_MIDI_CH), program_number].pack("C*"))
  wait(800)
end

def note_on(note_number, velocity)
  $file.write([(NOTE_ON  | PRA32_U2_MIDI_CH), note_number, velocity].pack("C*"))
end
