...
$file.printf("int32_t g_eg_attack_decay_release_coef_table[] = {\n  ")
(0..127 * (2 ** EG_TABLE_EXT_BITS) + 16 * (2 ** EG_TABLE_EXT_BITS)).each do |i|
  time = i
  ...
  if i == 127 * (2 ** EG_TABLE_EXT_BITS) + 16 * (2 ** EG_TABLE_EXT_BITS)
    $file.printf("\n")
