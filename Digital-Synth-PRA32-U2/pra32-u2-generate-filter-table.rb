...
def generate_filter_lpf_table(res_id, name, q)
  ...
  (0..((DATA_BYTE_MAX * 2) << FILTER_TABLE_EXTENSION_BITS) + 1).each do |i|
    ...
    f_0 = (2.0 ** ((f_idx / 2.0 / (1 << FILTER_TABLE_EXTENSION_BITS)) / (120.0 / OCTAVES))) * ((A4_FREQ * 2.0) * 16.0) * 2.0 / (2.0 ** (OCTAVES.to_f + 1.0))
    ...
    if i == (((DATA_BYTE_MAX * 2) << FILTER_TABLE_EXTENSION_BITS) + 1)
      $file.printf("\n")
    elsif i % 2 == (2 - 1)
      $file.printf("\n  ")
    else
