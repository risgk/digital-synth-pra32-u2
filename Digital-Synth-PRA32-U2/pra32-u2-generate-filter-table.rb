...
def generate_filter_table(res_id, name, q)
  $file.printf("const int32_t g_filter_table_%s[] = {\n  ", name)
...
(0..MAX_RES_ID).each do |res_id|
  generate_filter_table(res_id, res_id.to_s, Math.sqrt(2.0) ** ((res_id - (2.0 * (1 << FILTER_TABLE_RESO_EXT_BITS))) / (2.0 * (1 << FILTER_TABLE_RESO_EXT_BITS))))
end
...
(0..16 * (1 << FILTER_TABLE_RESO_EXT_BITS)).each do |res_index|
  ...
  $file.printf("g_filter_table_%-2d,", res_id)
